...
namespace HelixToolkit.Wpf
{
    ...
    public class MeshBuilder
    {
        ...
        /// </summary>
        private Int32Collection _Triangles;
        #endregion
        ...
        {
            this._Points = new Point3DCollection();
            this._Triangles = new Int32Collection();
            ...
            {
                this._TextureCoords = new PointCollection();
            }
        }
        ...
        public bool CreateTextureCoordinates
        {
            get
            {
                return this._TextureCoords != null;
            }

            set
            {
                if (value && this._TextureCoords == null)
                {
                    this._TextureCoords = new PointCollection();
                }
        ...
        public static IList<Point> GetCircle(int thetaDiv)
        {
            ...
            {
                var _circle = new PointCollection();
                for (int i = 0; i < thetaDiv; i++)
                {
                    double theta = Math.PI * 2 * ((double)i / (thetaDiv - 1));
                    _circle.Add(new Point(Math.Cos(theta), -Math.Sin(theta)));
                }
                _circle.Freeze();
                CircleCache.Add(thetaDiv, _circle);
                circle = _circle;
            }
        ...
        /// </param>
        public void AddBoundingBox(Rect3D boundingBox, double diameter, int thetaDiv = 10)
        {
            ...
            Action<Point3D, Point3D> addEdge = (c1, c2) => this.AddCylinder(c1, c2, diameter, false, false, 10);
        ...
        /// </remarks>
        public void AddCone(Point3D origin, Vector3D direction, double baseRadius, double topRadius,
            double height, bool baseCap, bool topCap, int thetaDiv,
            Transform3D originTransform, Transform3D destinationTransform)
        {
            ...
            _points.Add(new Point(0, baseRadius));
            _points.Add(new Point(height, topRadius));
            if (topCap)
            {
                _points.Add(new Point(height, 0));
            }

            IList<double> _txtPts =
                baseCap && topCap
                ? new List<double>(new double[] { 0, 0.2d, 0.8d, 1.0d })
                : baseCap
                ? new List<double>(new double[] { 0, 0.8d, 1.0d })
                : topCap
                ? new List<double>(new double[] { 0, 0.2d, 1.0d })
                : new List<double>(new double[] { 0, 1.0d });
            this.AddRevolvedGeometry(_points, _txtPts, origin, direction, thetaDiv, originTransform, destinationTransform,
                (baseCap ? 1 : 0));
        }
        ...
        public void AddCubeFace(Point3D center, Vector3D normal, Vector3D up, double dist, double width, double height)
        {
            ...
            {
                this._TextureCoords.Add(new Point(1, 1));
                this._TextureCoords.Add(new Point(0, 1));
                this._TextureCoords.Add(new Point(0, 0));
                this._TextureCoords.Add(new Point(1, 0));
            }

            this._Triangles.Add(i0 + 2);
            this._Triangles.Add(i0 + 1);
            this._Triangles.Add(i0 + 0);
            this._Triangles.Add(i0 + 0);
            this._Triangles.Add(i0 + 3);
            this._Triangles.Add(i0 + 2);
        }
        ...
        /// </remarks>
        public void AddCylinder(Point3D p1, Point3D p2, double diameter, bool baseCap, bool topCap, int thetaDiv)
        {
            Vector3D n = p2 - p1;
            double l = n.Length;
            n.Normalize();
            this.AddCone(p1, n, diameter / 2, diameter / 2, l, baseCap, topCap, thetaDiv,
                TranslateTransform3D.Identity, TranslateTransform3D.Identity);
        }
        ...
        /// Transformation to apply to origin points
        /// </param>
        /// <param name="destinationTransform">
        /// Transformation to apply to destination points
        /// </param>
        /// <remarks>
        /// See http://en.wikipedia.org/wiki/Cylinder_(geometry).
        /// </remarks>
        ...
        public void AddEdges(IList<Point3D> points, IList<int> edges, double diameter, int thetaDiv)
        {
            ...
            {
                this.AddCylinder(points[edges[i]], points[edges[i + 1]], diameter, false, false, thetaDiv);
            }
        }
        ...
        public void AddExtrudedGeometry(IList<Point> points, Vector3D xaxis, Point3D p0, Point3D p1)
        {
            ...
            int index0 = this._Points.Count;
            int np = 2 * points.Count;
            foreach (var p in points)
            {
                var v = (xaxis * p.X) + (ydirection * p.Y);
                this._Points.Add(p0 + v);
                this._Points.Add(p1 + v);
                v.Normalize();
                if (this._Normals != null)
                {
                    this._Normals.Add(v);
                ...
                this._Triangles.Add(i1);
                this._Triangles.Add(i2);
                this._Triangles.Add(index0);

                this._Triangles.Add(i1);
                this._Triangles.Add(i3);
                this._Triangles.Add(i2);
            }
        }
            ...
            IList<IList<Point>> textureCoordinateList)
        {
            ...
            for (int i = 0; i < positionsList.Count; i++)
            {
                ...
                // add texcoords
                if (this._TextureCoords != null && textureCoordinateList != null)
                {
                    var tc = textureCoordinateList[i];
                    foreach (var t in tc)
                    {
                        this._TextureCoords.Add(t);
                    }
                }
            }
        ...
        public void AddNode(Point3D position, Vector3D normal, Point textureCoordinate)
        {
            ...
            if (this._TextureCoords != null)
            {
                this._TextureCoords.Add(textureCoordinate);
            }
        }
        ...
        public void AddQuad(Point3D p0, Point3D p1, Point3D p2, Point3D p3, Point uv0, Point uv1, Point uv2, Point uv3)
        {
            ...
            if (this._TextureCoords != null)
            {
                this._TextureCoords.Add(uv0);
                this._TextureCoords.Add(uv1);
                this._TextureCoords.Add(uv2);
                this._TextureCoords.Add(uv3);
            }
            ...
            IList<Point3D> quadPositions, IList<Vector3D> quadNormals, IList<Point> quadTextureCoordinates)
        {
            ...
            if (this._TextureCoords != null && quadTextureCoordinates == null)
            {
            ...
            int indexEnd = this._Points.Count;
            for (int i = index0; i + 3 < indexEnd; i++)
            {
                this._Triangles.Add(i);
                this._Triangles.Add(i + 1);
                this._Triangles.Add(i + 2);

                this._Triangles.Add(i + 2);
                this._Triangles.Add(i + 3);
                this._Triangles.Add(i);
            }
        }
            ...
            if (this._TextureCoords != null)
            {
                if (texCoords != null)
                {
                    for (int i = 0; i < rows; i++)
                    {
                        for (int j = 0; j < columns; j++)
                        {
                            this._TextureCoords.Add(texCoords[i, j]);
                        }
                    }
                }
                else
        ...
        public void AddRegularIcosahedron(Point3D center, double radius, bool shareVertices)
        {
            ...
            if (shareVertices)
            {
                ...
                {
                    this._Points.Add(center + (v * radius));
                }

                foreach (int i in icosahedronIndices)
                {
                    this._Triangles.Add(index0 + i);
                }
            }
            else
            {
        ...
        private List<double> LinearTexturePoints(IList<Point> points)
        {
            return LinearTexturePoints(points, 0, points.Count - 1);
        }

        private List<double> LinearTexturePoints(IList<Point> points, int first, int last)
        {
            var _sum = 0d;
            ...
            {
                _sum += (points[_px] - points[_px + 1]).Length;
                _txtPts.Add(_sum);
            }
        ...
        {
            AddRevolvedGeometry(points, LinearTexturePoints(points), origin, direction, thetaDiv, TranslateTransform3D.Identity, TranslateTransform3D.Identity, 0);
        }

        /// <summary>Adds a surface of revolution.</summary>
        /// <param name="points">The points (x coordinates are radius, y coordinates are distance from the origin along the axis of revolution)</param>
        ...
        /// <remarks>See http://en.wikipedia.org/wiki/Surface_of_revolution.</remarks>
        private void AddRevolvedGeometry(IList<Point> points, IList<double> texturePoints, Point3D origin, Vector3D direction, int thetaDiv,
            Transform3D originTransform, Transform3D destinationTransform, int maxOriginIndex)
        {
            direction.Normalize();
            ...
            var _circle = GetCircle(thetaDiv);

            int _startIdx = this._Points.Count;
            int _pointCount = points.Count;

            int _totalNodes = (points.Count - 1) * 2 * thetaDiv;
            int _rowNodeCount = (points.Count - 1) * 2;

            for (int _sector = 0; _sector < thetaDiv; _sector++)
            {
                var w = (v * _circle[_sector].X) + (u * _circle[_sector].Y);

                Func<Point3D, int, Point3D> _transform = (pt, idx) =>
                    (idx > maxOriginIndex)
                    ? destinationTransform.Transform(pt)
                    : originTransform.Transform(pt);

                for (int _ptIdx = 0; _ptIdx + 1 < _pointCount; _ptIdx++)
                {
                    // Add segment
                    var q1 = _transform(origin + (direction * points[_ptIdx].X) + (w * points[_ptIdx].Y), _ptIdx);
                    var q2 = _transform(origin + (direction * points[_ptIdx + 1].X) + (w * points[_ptIdx + 1].Y), _ptIdx + 1);

                    // todo:should not add segment if q1==q2 (corner point)
                    // const double eps = 1e-6;
                    // if (Point3D.Subtract(q1, q2).LengthSquared < eps)
                    // continue;
                    double tx = points[_ptIdx + 1].X - points[_ptIdx].X;
                    double ty = points[_ptIdx + 1].Y - points[_ptIdx].Y;
                    ...
                    if (this._Normals != null)
                    {
                        var _norm = (-direction * ty) + (w * tx);
                        _norm.Normalize();
                        this._Normals.Add(_norm);
                        this._Normals.Add(_norm);
                    }

                    if (this._TextureCoords != null)
                    {
                        this._TextureCoords.Add(new Point((double)_sector / (thetaDiv - 1), texturePoints[_ptIdx]));
                        this._TextureCoords.Add(new Point((double)_sector / (thetaDiv - 1), texturePoints[_ptIdx + 1]));
                    }

                    int _tri0 = _startIdx + (_sector * _rowNodeCount) + (_ptIdx * 2);
                    int _tri1 = _tri0 + 1;
                    int _tri2 = _startIdx + ((((_sector + 1) * _rowNodeCount) + (_ptIdx * 2)) % _totalNodes);
                    int _tri3 = _tri2 + 1;

                    this._Triangles.Add(_tri1);
                    this._Triangles.Add(_tri0);
                    this._Triangles.Add(_tri2);

                    this._Triangles.Add(_tri1);
                    this._Triangles.Add(_tri2);
                    this._Triangles.Add(_tri3);
                }
            }
        }
        ...
        {
            int p0 = this._Points.Count;
            this.Append(GetUnitSphere(subdivisions));
            int p1 = this._Points.Count;
            for (int i = p0; i < p1; i++)
            {
                this._Points[i] = center + (radius * this._Points[i].ToVector3D());
            }
        }
        ...
        public void AddTriangleFan(IList<int> vertices)
        {
            ...
            {
                this._Triangles.Add(vertices[0]);
                this._Triangles.Add(vertices[i + 1]);
                this._Triangles.Add(vertices[i + 2]);
            }
        }
            ...
            IList<Point3D> fanPositions, IList<Vector3D> fanNormals = null, IList<Point> fanTextureCoordinates = null)
        {
            ...
            if (this._TextureCoords != null && this._TextureCoords == null)
            {
            ...
            if (this._TextureCoords != null && fanTextureCoordinates != null)
            {
            ...
            IList<Point> stripTextureCoordinates = null)
        {
            ...
            if (this._TextureCoords != null && stripTextureCoordinates == null)
            {
            ...
            {
                this._Points.Add(stripPositions[i]);
                if (this._Normals != null && stripNormals != null)
                {
                ...
                if (this._TextureCoords != null && stripTextureCoordinates != null)
                {
                    this._TextureCoords.Add(stripTextureCoordinates[i]);
                }
            }
        ...
        public void AddPolygon(IList<int> vertexIndices)
        {
            ...
            for (int i = 0; i + 2 < n; i++)
            {
                this._Triangles.Add(vertexIndices[0]);
                this._Triangles.Add(vertexIndices[i + 1]);
                this._Triangles.Add(vertexIndices[i + 2]);
            }
        }
        ...
        {
            var points = vertexIndices.Select(vi => this._Points[vi]).ToList();
            ...
            if (triangulatedIndices != null)
            {
                ...
                {
                    this._Triangles.Add(vertexIndices[i]);
                }
            }
        }
            ...
            IList<Point> triangleTextureCoordinates = null)
        {
            ...
            if (this._TextureCoords != null && triangleTextureCoordinates == null)
            {
        ...
        public void Append(MeshBuilder mesh)
        {
            ...
            this.Append(mesh._Points, mesh._Triangles, mesh._Normals, mesh._TextureCoords);
        }
            ...
            IList<Point> textureCoordinatesToAppend = null)
        {
            ...
            if (this._TextureCoords != null && textureCoordinatesToAppend == null)
            {
        ...
        public void ChamferCorner(Point3D p, double d, double eps = 1e-6, IList<Point3D> chamferPoints = null)
        {
            ...
            int index0 = this._Points.Count;
            this._Points.Add(newCornerPoint);

            var plane = new Plane3D(newCornerPoint, cornerNormal);

            int ntri = this._Triangles.Count;
            ...
            for (int i = 0; i < ntri; i += 3)
            {
                ...
                int i1 = i + 1;
                int i2 = i + 2;
                var p0 = this._Points[this._Triangles[i0]];
                var p1 = this._Points[this._Triangles[i1]];
                var p2 = this._Points[this._Triangles[i2]];
                ...
                p0 = this._Points[this._Triangles[i0]];
                p1 = this._Points[this._Triangles[i1]];
                p2 = this._Points[this._Triangles[i2]];
                ...
                this._Points[this._Triangles[i01]] = p01.Value;

                int i02 = this._Points.Count;
                this._Points.Add(p02.Value);

                // add a new triangle for the other chamfer point
                this._Triangles.Add(i01);
                this._Triangles.Add(i2);
                this._Triangles.Add(i02);
        ...
        {
            if (this._Points.Count > 20000)
            {
                Trace.WriteLine(string.Format("Too many positions ({0}).", this._Points.Count));
            }

            if (this._Triangles.Count > 60002)
            {
                Trace.WriteLine(string.Format("Too many triangle indices ({0}).", this._Triangles.Count));
            }
        }
        ...
        {
            if (this._Normals != null && this._Points.Count != this._Normals.Count)
            {
                throw new InvalidOperationException(WrongNumberOfNormals);
            }

            if (this._TextureCoords != null && this._Points.Count != this._TextureCoords.Count)
            {
                throw new InvalidOperationException(WrongNumberOfTextureCoordinates);
            }
            ...
            {
                Positions = this._Points,
                TriangleIndices = this._Triangles,
                Normals = this._Normals,
                TextureCoordinates = this._TextureCoords
            };
            if (freeze)
            {
                mg.Freeze();
        ...
        private static MeshGeometry3D GetUnitSphere(int subdivisions)
        {
            ...
            for (int i = 0; i < mb._Points.Count; i++)
            {
            ...
            for (int i = 0; i < rows; i++)
            {
                ...
                for (int j = 0; j < columns; j++)
                {
                    ...
                    var u = Point3D.Subtract(
                        this._Points[index0 + (i1 * columns) + j0], this._Points[index0 + (i0 * columns) + j0]);
                    var v = Point3D.Subtract(
                        this._Points[index0 + (i0 * columns) + j1], this._Points[index0 + (i0 * columns) + j0]);
        ...
        private void AddRectangularMeshTextureCoordinates(int rows, int columns)
        {
            for (int i = 0; i < rows; i++)
            {
                ...
                for (int j = 0; j < columns; j++)
                {
                    ...
                    this._TextureCoords.Add(new Point(u, v));
                }
            }
        }
        ...
        private void AddRectangularMeshTriangleIndices(int index0, int rows, int columns, bool isSpherical = false)
        {
            for (int i = 0; i < rows - 1; i++)
            {
                for (int j = 0; j < columns - 1; j++)
                {
                    ...
                    if (!isSpherical || i > 0)
                    {
                        this._Triangles.Add(index0 + ij);
                        this._Triangles.Add(index0 + ij + 1 + columns);
                        this._Triangles.Add(index0 + ij + 1);
                    }
                    ...
                    if (!isSpherical || i < rows - 2)
                    {
                        this._Triangles.Add(index0 + ij + 1 + columns);
                        this._Triangles.Add(index0 + ij);
                        this._Triangles.Add(index0 + ij + columns);
                    }
                }
            }
        }
            ...
            int index0, int rows, int columns, bool rowsClosed, bool columnsClosed)
        {
            ...
            for (int i = 0; i < m2; i++)
            {
                for (int j = 0; j < n2; j++)
                {
                    ...
                    this._Triangles.Add(i00);
                    this._Triangles.Add(i11);
                    this._Triangles.Add(i01);

                    this._Triangles.Add(i11);
                    this._Triangles.Add(i00);
                    this._Triangles.Add(i10);
                }
            }
        }
        ...
        private void NoSharedVertices()
        {
            ...
            for (int i = 0; i < this._Triangles.Count; i += 3)
            {
                ...
                int i1 = i + 1;
                int i2 = i + 2;
                int index0 = this._Triangles[i0];
                int index1 = this._Triangles[i1];
                int index2 = this._Triangles[i2];
                var p0 = this._Points[index0];
                ...
                if (tc != null)
                {
                    tc.Add(this._TextureCoords[index0]);
                    tc.Add(this._TextureCoords[index1]);
                    tc.Add(this._TextureCoords[index2]);
                }
            }
