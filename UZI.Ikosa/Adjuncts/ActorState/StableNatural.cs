...
{
    [Serializable]
    public class StableNatural : ActorStateBase, ITrackTime, IRecoveryAdjunct
    {
        public StableNatural(object source)
            : base(source)
        {
        ...
        {
            return new StableNatural(Source);
        }
        ...
        public void TrackTime(double timeVal, TimeValTransition direction)
        {
            if ((timeVal >= _NextCheck) && (direction == TimeValTransition.Entering))
            {
                ...
                var _rollStep = new RollSplitStep((CoreProcess)null, @"Creature.Stable",
                    string.Format(@"10% recover to disabled, or lose 1 HP ({0})", _critter != null ? _critter.Name : Anchor.ID.ToString()),
                    new DieRoller(100));
                var _process = new CoreProcess(_rollStep, @"Recovery Chance");
                _rollStep.PossibleSteps.Add(10, new ActorStateChangeStep(_process, this, new NotHealing(Source), _critter));
