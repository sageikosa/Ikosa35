...
{
    [
        WeaponHead(@"1d4", DamageType.Bludgeoning, 20, 2, typeof(Materials.WoodMaterial), Lethality.NormallyLethal),
        ItemInfo(@"Shield, Heavy Wooden", @"AR:+2 Check:-2 SpellFail:15%", @"heavy_wooden_shield"),
        Serializable
    ]
    public class HeavyWoodenShield : ShieldBase, IMeleeWeapon
    {
        public HeavyWoodenShield()
            : base("Shield, Heavy Wooden", false, 2, -2, 15, false)
        {
            Init(7m, 10d, Materials.WoodMaterial.Static, 15);
            GetHead();
            GetDisabler();
        }
        ...
        protected override string ClassIconKey => @"heavy_wooden_shield";
    }
}
