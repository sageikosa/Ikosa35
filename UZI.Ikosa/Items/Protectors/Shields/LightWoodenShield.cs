...
{
    [
        WeaponHead(@"1d3", DamageType.Bludgeoning, 20, 2, typeof(Materials.WoodMaterial), Lethality.NormallyLethal),
        ItemInfo(@"Shield, Light Wooden", @"AR:+1 Check:-1 SpellFail:5%", @"light_wooden_shield"),
        Serializable
    ]
    /// <summary>
    /// Shield that lets you use the hand, but not for wielding.
    /// </summary>
    public class LightWoodenShield : ShieldBase, IMeleeWeapon
    {
        public LightWoodenShield()
            : base("Shield, Light Wooden", false, 1, -1, 5, true)
        {
            Init(3m, 5d, Materials.WoodMaterial.Static, 7);
            GetHead();
            GetDisabler();
        }
        ...
        {
            if (action is ISupplyAttackAction)
            {
                var _atk = action as ISupplyAttackAction;
        ...
        protected override string ClassIconKey => @"light_wooden_shield";
