...
{
    [Serializable]
    public class Blur : SpellDef, ISpellMode, IDurableCapable, ISaveCapable, IProcessFeedback
    {
        public override string DisplayName => @"Blur";
        public override string Description => @"Target gains concealment (20% miss chance against attack rolls)";
        ...
        public void ProcessFeedback(Interaction workSet)
        {
                ...
                && (_atk.Attacker is Creature _attacker))
            {
                ...
                {
                    // True-sight, Blindsight?
                    if (_attacker.Senses.BestSenses
                        .Where(_s => _s.IgnoresVisualEffects && _s.UsesLineOfEffect && _s.ForTargeting)
                ...
                if (_miss.PercentRolled <= 20)
                {
                    // rolled a miss, see if a second change is possible
                    if (!_miss.SecondRoll
                       && _attacker.Feats.Contains(typeof(BlindFight)))
                    {
                        // yes, so do-over
