...
{
    [Serializable]
    public class BalanceDamageStep : PreReqListStepBase
    {
        public BalanceDamageStep(Interaction workSet, SuccessCheck check)
            : base((CoreProcess)null)
        {
            // workset should include actor needing to make the save as the target
            _PendingPreRequisites.Enqueue(
                new SuccessCheckPrerequisite(this, workSet, @"BalanceCheck", @"Balance Check from Damage", check, false));
        }
        ...
        protected override bool OnDoStep()
        {
            ...
            if (_checkPre != null)
            {
                ...
                if (!_checkPre.Success)
                {
                    ...
                    if (_located != null)
                    {
                        EnqueueNotify(new CheckResultNotify(_id, @"Movement", false, new Description(@"Balance", @"Failure")), _id);
                        AppendFollowing(new FallingStartStep(this, _located.Locator, 500, 0, 1));
                    }
                }
                else
                {
                    EnqueueNotify(new CheckResultNotify(_id, @"Movement", true, new Description(@"Balance", @"Success")), _id);
                }
            }
            return true;
        }
