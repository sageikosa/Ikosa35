...
{
    [Serializable]
    public class CornerPivotPortalHandler : IProcessFeedback
    {
        #region IProcessFeedback Members

        public void ProcessFeedback(Interaction workSet)
        {
            if (workSet.Target is CornerPivotPortal _portal)
            {
                ...
                else if (workSet.InteractData is VisualPresentationData)
                {
                    ...
                    foreach (var _vmdBack in workSet.Feedback.OfType<VisualModelFeedback>())
                    {
                        ...
                        _vmdBack.ModelPresentation.Pivot = _portal.OverridePivot();
                        _vmdBack.ModelPresentation.Tilt = _portal.OverrideTilt();
