...
namespace Uzi.Ikosa.Time
{
    ...
    {
        public Sleep(IActionSource source, string orderKey)
            : base(source, new ActionTime(Hour.UnitFactor, TimeType.TimelineScheduling), false, false, orderKey)
        {
        }

        public override string Key => @"Timeline.Sleeping";
        public override string DisplayName(CoreActor actor) => @"Sleeping";
        ...
        public override ObservedActivityInfo GetActivityInfo(CoreActivity activity, CoreActor observer)
            => ObservedActivityInfoFactory.CreateInfo(@"Sleeping", activity.Actor, observer);
                ...
                if (activity.Actor is Creature _critter)
                {
                    ...
                    if (_hours.Value > 0)
                    {
                        ...
                        Budget.NextActivity = new CoreActivity(
                            _critter,
                            new Sleep(ActionSource, OrderKey),
                            activity.Targets);
                    }
                    return activity.GetActivityResultNotifyStep($@"Slept 1 hour");
                }
            }
            return activity.GetActivityResultNotifyStep(@"Improper sleep parameters");
        }
    }
}
