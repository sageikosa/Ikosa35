...
namespace Uzi.Visualize.IkosaPackaging
{
    ...
    /// </summary>
    public class ResourceManager : BasePart, ICorePartNameManager, IHideCorePackagePartsFolder, IResolveIcon,
        IResolveBitmapImage, IResolveMaterial, IResolveFragment, IResolveModel3D, IResolveBrushCollection
    {
        ...
        public const string ResourceManagerRelation = @"http://pack.guildsmanship.com/resourcemanager";

        #region construction
        /// <summary>Bind to an existing part</summary>
        public ResourceManager(ICorePartNameManager manager, PackagePart part, string name)
            : base(manager, part, name)
        {
            ...
            // images
            if (part.RelationshipExists(@"Images"))
            {
                var _imgRel = part.GetRelationship(@"Images");
                _Images = this.GetBasePart(_imgRel) as CorePackagePartsFolder;
                if (_Images == null)
            ...
            // brushsets
            if (part.RelationshipExists(@"BrushSets"))
            {
                var _brushSetRel = part.GetRelationship(@"BrushSets");
                _BrushSets = this.GetBasePart(_brushSetRel) as CorePackagePartsFolder;
                if (_BrushSets == null)
            ...
            // fragments
            if (part.RelationshipExists(@"Fragments"))
            {
                var _fragRel = part.GetRelationship(@"Fragments");
                _Fragments = this.GetBasePart(_fragRel) as CorePackagePartsFolder;
                if (_Fragments == null)
            ...
            // models
            if (part.RelationshipExists(@"Models"))
            {
                var _mdlRel = part.GetRelationship(@"Models");
                _Models = this.GetBasePart(_mdlRel) as CorePackagePartsFolder;
                if (_Models == null)
            ...
            // icons
            if (part.RelationshipExists(@"Icons"))
            {
                var _icoRel = part.GetRelationship(@"Icons");
                _Icons = this.GetBasePart(_icoRel) as CorePackagePartsFolder;
                if (_Icons == null)
        ...
        public ResourceManager(ICorePartNameManager manager, string name)
            : base(manager, name)
        {
        ...
        public override string TypeName => typeof(ResourceManager).FullName;
        ...
        public override void Save(Package parent)
        {
            ...
            parent.CreateRelationship(_content, TargetMode.Internal, ResourceManagerRelation, Name);
